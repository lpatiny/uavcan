...
describe('UAVCANCodec', () => {
    ...
    function testfnMultiPacket() {
      return new Promise(function (resolve, reject) {
        ...
        for (let i = 0; i < canPayloadFull.length; i++) {
          myCodecMulti.decode(
            Buffer.from(canId),
            Buffer.from(canPayloadFull[i])
          );
        }
      });
    }

    expect(testfnMultiPacket()).resolves.toStrictEqual({
      value: 59.0,
      key: [100, 101, 102],
      keyStr: 'def'
    });
  ...
  it('bad_data', () => {
    ...
    let canPayload = Buffer.from([
      [0x00, 0x01, 0x1d, 0x00, 0x00, 0x00, 0x00, 0x00],
      [0x00, 0x00, 0x64, 0x72]
    ]);
  ...
  it('multi_response', () => {
    ...
    let assembled = false;
    for (let i = 0; i < canPayloadFull.length; i++) {
      assembled = myCodec.assembleTransfer(
        Buffer.from(canPayloadFull[i]),
        canId
      );
    }
